...
/**
 * the class handles all actions in the customersView scenes - view, add and
...
public class CustomersController extends DashboardController implements DataFormatter {
	...
	/**
	 * hides the empty list view, and populates the specified grid pane with all
	 * customer details read from the file called by initialize() method
	 */
	...
	/**
	 * on click of the button, validates user input, creates a new customer and add
	 * to customer file
	 * 
	 * @param e
	 * @throws IOException
	...
	public void addNewCustomerBtnListener(ActionEvent e) throws IOException {
		...
		if (name.getText() == "" || email.getText() == "" || phone.getText() == "" || dob.getValue() == null) {
			...
			alert.setContentText(
					"Please enter a correct phone number: country code starting with + and, between 8 to 14 digits");
			alert.show();
	...
	/**
	 * validates user input in the form fields and updates the attribute values for
	 * the customer currently being updated
	 * 
	 * @param e
	 * @throws IOException
	 */
	public void updateCustomerBtnListener(ActionEvent e) throws IOException {
		...
		if (name.getText() == "" || email.getText() == "" || phone.getText() == "" || dob == null) {
			...
			try {
				// check if updates do not match existing customer
				...
				if (!customerExists) {
					...
					alert.setTitle("Error updating customer details");
					alert.setContentText(
							"Cannot update customer as another customer already exists with these details");
					alert.show();
				}
			} catch (Exception exception) {
				alert.setAlertType(AlertType.ERROR);
