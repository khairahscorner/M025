...
/**
 * the class is used to display a list of all properties, filter & sort options,
 * view and edit the details of a specifc property
 * 
...
public class PropertiesController extends DashboardController implements DataFormatter {
	...
	public void initialize() {
		try {
			...
			/**
			 * get the first half of all postcodes and save the unique keys as values for
			 * the "filter by postcode" option; this will return more matched results as
			 * opposed to specific postcodes
			 */
	...
	/**
	 * populates the specified grid pane with all property details read from the
	 * file called by initialize() method
	 */
	...
	private void viewPropertyDetailsListener(String currPptyId) {
		...
		pptyDetailsArea.setText(currPpty.getPropertyDetails());

		// display closest places of interest
		List<Landmark> allLandmarks = lList.getLandmarks();
		pptyDetailsArea.appendText(PropertyActions.getClosestLandmarksDistance(currPpty, allLandmarks));
	...
	/**
	 * filter properties list based on number of bedrooms (3+ for 3 or more
	 * bedrooms)
	 */
	...
	/**
	 * filter properties list based on number of bathrooms (3+ for 3 or more
	 * bathrooms)
	 */
	...
	/**
	 * switches the details view to edit property and populates the form fields with
	 * the property values
	 */
	...
	/**
	 * validates input, updates property details in list and rewrites the list to
	...
	public void updatePptyDetailsListener() throws IOException {
		...
		if (editType.getText() == "" || editPostcode.getText() == "" || editFurnishing.getValue() == null
				|| editGarden.getValue() == null) {
			alert.setAlertType(AlertType.ERROR);
			alert.setTitle("Error updating property");
			...
			try {	
				...
				for (Property p : pList.getProperties().values()) {
					if (p.getType().equals(editType.getText()) && p.getPostcode().equals(editPostcode.getText())
							&& p.getFurnishedStatus().equals(editFurnishing.getValue()) && p.getLatitude() == latitude
							&& p.getLongitude() == longitude && p.getSize() == pptySize && p.getBedrooms() == bedrooms
							&& p.getBathrooms() == bathrooms) {
						propertyExists = true;
						break;
					}
				}
